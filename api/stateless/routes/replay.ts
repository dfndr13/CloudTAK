...
//
// Recording hook (Recorder, in ../../stateful/lib/replay-recorder.ts) taps
...
//
// Recorder lives in the stateful process (it needs ConnectionPool), while
// these routes are stateless. There's no Hub RPC for recording control, so
// ConfigStateless carries a direct `recorder` reference passed at boot only
// when co-located with a stateful process ('both' mode) - see
// stateless/config.ts and index.ts. requireRecorder() below guards every
// route that needs it.
//
...
// broadcasts replayed CoT to the requesting user's own live browser session
// only (config.hub.submitCots(), no tak.write()) - private preview, nothing
// sent to real TAK Server, nobody else sees it unless they import the
// exported event file into their own CloudTAK and replay it locally.
...
function requireRecorder(config: ConfigStateless) {
    if (!config.recorder) {
        throw new Err(501, null, 'Recording control requires a co-located stateful process (CLOUDTAK_Server_Mode=both)');
    }
...
export default async function router(schema: Schema, config: ConfigStateless) {
    ...
    await schema.post('/replay/record/start', {
        ...
        try {
            ...
            const recorder = requireRecorder(config);
            const id = await recorder.start(req.body.name, user.email);
    ...
    await schema.post('/replay/record/stop', {
        ...
        try {
            ...
            const recorder = requireRecorder(config);
            const wasActive = recorder.active();
            await recorder.stop();
    ...
    await schema.get('/replay/record/status', {
        ...
        try {
            ...
            res.json({
                ...
                event: recorder.activeEvent || undefined,
            });
        } catch (err) {
            Err.respond(err, res);
        }
    ...
    await schema.post('/replay/record/feature', {
        ...
        try {
            ...
            await recorder.recordDirect(req.body.id, cot.type(), xml);
    ...
    await schema.delete('/replay/record/feature/:id', {
        ...
        try {
            ...
            await recorder.recordRemoval(req.params.id);
