// api/stateless/lib/replay-player.ts
//
// SIMPLIFIED: no dedicated layer certs, no tak.write() to real TAK Server.
// Playback broadcasts to the requesting user's OWN browser session only, via
// config.hub.submitCots({ connection: username, ... }) - the same identity
// they already have for normal live use. Nothing is written to TAK Server;
// nobody else sees replayed data. This is a private preview for the user who
// started playback, matching "just let me see what this looks like" rather
// than a shared/published feature - the layer-cert approach can be revisited
// later if/when this needs to be visible to other users or persist in TAK
// Server.
//
// Publishing goes through the Hub abstraction (submitCots), not a direct
// ConnectionPool reference, because this class lives in the stateless
// process/module graph - ConfigStateless has no .conns, only .hub. In "both"
// mode config.hub is the stateful process's LocalHub in-process (no RPC
// hop); in a true split deployment it's a RemoteHub making an HTTP call.
// Either way this class doesn't need to know or care which.
...
export default class Player {
    ...
    private async publishStateAt(s: PlaybackControl, opts: { fullSnapshot?: boolean } = {}) {
        // Mirrors the pre-check submitCots({ ensureProfile: true }) would
        // otherwise do internally, so a user with no cert/key configured
        // never advances lastPublishedAt below - once their connection is
        // configured, playback picks up from where it left off rather than
        // silently losing whatever window elapsed in the meantime.
        ...
        try {
            ...
            console.error(`[replay] failed to publish playback CoTs for ${s.username}:`, err);
        }
    }
}
